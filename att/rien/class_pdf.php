...
if (isset($_GET['id_loti']))
{
   ...
   //Recuperation du plus grand element de la base
    $req="SELECT num_ilot FROM ilot WHERE lotissement.id_loti=:aid_loti LIMIT 1";
    $stmt=$pdo->prepare($req);
    $stmt->bindParam(':aid_loti',$_GET['id_loti'],PDO::PARAM_INT);
    $stmt->execute();
    ...
    while ($ligne=$stmt->fetch(PDO::FETCH_ASSOC) )
    {
        ...
        while ($adeb_lot<=$afin_lot  && $num_d<$num_f)
        {   
        ...
        $pdf->AddPage('L', 'A4');
            // creation d'une page
            $compteur=0;
            while ($compteur<3 && $num_d<$num_f && $adeb_lot<=$afin_lot) 
            {
        ...
        <tr class="center">
          ...
          if($adeb_lot>=$afin_lot || $adeb_lot==0 || $adeb_lot==$afin_lot)
                {
                    $ligne=$stmt->fetch(PDO::FETCH_ASSOC);
                    $num_d++;
                }
